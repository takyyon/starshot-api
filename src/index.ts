...
app.get("/", async (req: Request, res: Response) => {
  ...
  if (!org || typeof org !== "string") {
    ...
    try {
      await runAndReturnAnalysis(res, org, repo, azd, branch);
    } catch (ex) {
      res.status(500);
...
const runAndReturnAnalysis = (
  ...
  return getFrameworks(org, repo, branch).then((frameworks) => {
    getRecommendation(frameworks, org, repo, azd, branch)
      .then((recommendation) => {
        res.status(200);
        res.json({
          frameworks,
